  ...
  case elem of
    ...
    RowAddition i j c ->
      let ithRow = filter ((== i) . fst . fst) (A.assocs inner)
          jthRowRaw = map snd $ filter ((== j) . fst . fst) (A.assocs inner)
       in Matrix $
          inner A.//
...
makePivotOneColHelper n oracle@(Oracle elems mat (maxRow, _) pivotIdx@(pivRow, _) (_, col) _)
  ...
  | otherwise =
    let val = addInv (mat |.| (n, col)) ~* pivot
     ...
     in if val == addIdent
          then makePivotOneColHelper (n + 1) oracle {getLocation = (n + 1, col)}
    ...
    helper k oracle@(Oracle elems mat (row, _) _ _ _)
      ...
      | isZeroRow k oracle =
        let elem = RowExchange k (k + 1)
         in helper (k + 1) $
            oracle {getElemOps = elem : elems, getMatrix = doElemRowOp elem mat}
